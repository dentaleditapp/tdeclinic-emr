...
# ---------------------------------
# 🔥 USE ONLY POSTGRESQL (Render)
# ---------------------------------
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "postgresql://tdeclinic_user:"
    "0Xp7rg0zCz7VXk5pyKfaS1bnEbjuRjJw"
    "@dpg-d4ec6rgdl3ps73bmlarg-a.frankfurt-postgres.render.com/"
    "tdeclinic"
)
...
class Treatment(db.Model):
    ...
    # -----------------------------
    treatment_category = db.Column(db.String(100))
    treatment_type = db.Column(db.String(150))
    date = db.Column(db.String(20), default=lambda: datetime.now().strftime("%Y-%m-%d"))
    notes = db.Column(db.Text)
    post_op = db.Column(db.Text)
    ...
    multi_tooth = db.Column(db.String(200))
    ...
    # 🌟 EXTRACTIONS & SURGERY
    # -----------------------------
    extraction_type = db.Column(db.String(100))
    ...
    sectioning_details = db.Column(db.Text)
...
def add_treatment(patient_id):
    ...
    if request.method == "POST":
        ...
        for canal in canal_list:
            ...
            if wl_val:
                wl_pairs.append(f"{canal}:{wl_val}")
            if maf_val:
                maf_pairs.append(f"{canal}:{maf_val}")
        ...
        bone_removal = bool(request.form.get("bone_removal"))
        sectioning = bool(request.form.get("tooth_sectioning"))
        sectioning_details = request.form.get("sectioning_details")
        post_op = request.form.get("post_op")

        # ---------------------------------------
        # CREATE TREATMENT OBJECT (FIXED)
        # ---------------------------------------
        treatment = Treatment(
            patient_id=patient.id,
            case_id=case_fk,          # 🔥 integer FK, NOT STRING
        ...
        payment = Payment(
            ...
            case_id=case_fk,       # 🔥 FIXED: was string before
            date=today_str,
            treatment_fee=fee,
            amount_paid=paid,
        ...
        # ---------------------------------------
        # FILE UPLOAD FIX (uses integer FK)
        # ---------------------------------------
...
if __name__ == "__main__":
    # PostgreSQL → create tables on first run
    with app.app_context():
        db.create_all()
